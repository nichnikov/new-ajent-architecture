...
class YandexSearchAPIClient:
    ...
    def _get_iam_token_from_oauth(self, oauth_token: str) -> IamTokenResponse:
        ...
        """
        try:
            response = requests.post(self.IAM_TOKEN_URL, json={"yandexPassportOauthToken": oauth_token}, timeout=10)
    ...
    def search(
        ...
        *,
        ...
        headers = {"Authorization": f"Bearer {self._iam_token}", "Content-Type": "application/json"}
        ...
        try:
            response = requests.post(self.BASE_URL, headers=headers, data=json.dumps(body), timeout=response_timeout)
            response.raise_for_status()
    ...
    def _check_operation_status(self, operation_id: str) -> Dict:
        ...
        try:
            response = requests.get(f"{self.OPERATION_URL}{operation_id}", headers=headers, timeout=30)
            response.raise_for_status()
    ...
    def get_links(
        ...
        results = self.search_and_wait(
            query_text, search_type=search_type, n_links=n_links, region=region, max_wait=max_wait, interval=interval
        )
