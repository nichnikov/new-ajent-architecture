...
from . import IamTokenResponse, Region, ResponseFormat, SearchType, YandexSearchAPIClient
from .exceptions import YandexAuthError, YandexSearchAPIError, YandexSearchTimeoutError
...
class AsyncYandexSearchAPIClient(YandexSearchAPIClient):
    ...
    async def _get_iam_token_from_oauth(self, oauth_token: str) -> IamTokenResponse:
        ...
        try:
            response = await self._client.post(
                self.IAM_TOKEN_URL, json={"yandexPassportOauthToken": oauth_token}, timeout=10
            )
    ...
    async def search(
        ...
        *,
        ...
        """
        headers = {"Authorization": f"Bearer {await self._iam_token}", "Content-Type": "application/json"}
        ...
        try:
            response = await self._client.post(self.BASE_URL, headers=headers, json=body, timeout=response_timeout)
            response.raise_for_status()
    ...
    async def _check_operation_status(self, operation_id: str) -> Dict:
        ...
        """
        headers = {"Authorization": f"Bearer {await self._iam_token}"}

        try:
            response = await self._client.get(f"{self.OPERATION_URL}{operation_id}", headers=headers, timeout=30)
            response.raise_for_status()
    ...
    async def search_and_wait(
        ...
        *,
        ...
        """
        operation_id = await self.search(query_text, search_type=search_type, n_links=n_links)
    ...
    async def get_links(
        ...
        results = await self.search_and_wait(
            query_text, search_type=search_type, n_links=n_links, max_wait=max_wait, interval=interval
        )
